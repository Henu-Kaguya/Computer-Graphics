...
A）`p->x += p->dx;`　　B）`p->x += 1.0f / p->dx;`  
C）`p->x = p->yMax;`　　D）`p->x += 1.0f;`
...
A）`!ColorEqual(current, boundaryColor)`　　B）`ColorEqual(current, boundaryColor)`  
C）`ColorEqual(current, fillColor)`　　D）`!ColorEqual(current, fillColor)`

**3.** (3) 处应填写的判断未被填充色的条件为：（　　）

A）`ColorEqual(current, boundaryColor)`　　B）`!ColorEqual(current, fillColor)`  
C）`ColorEqual(current, fillColor)`　　D）`!ColorEqual(current, boundaryColor)`

**4.** (4) 处应填写的递归调用函数名为：（　　）

A）`SetPixelColor`　　B）`FloodFill4`  
C）`FloodFill8`　　D）`UpdateAET`
...
A）`vis[left][p.y] != 1`　　B）`vis[left][p.y] == 1`  
C）`left >= 0`　　D）`vis[left][p.y] == 0`

**6.** (6) 处循环向右填充非边界像素的逻辑判定为：（　　）

A）`vis[right][p.y] == 1`　　B）`vis[right][p.y] != 1`  
C）`right < 400`　　D）`vis[right][p.y] == 0`

**7.** (7) 处传入的下一扫描线位置为：（　　）

A）`p.y`　　B）`p.y - 1`　　C）`p.y + 1`　　D）`p.x - 1`

**8.** (8) 处传入的上一扫描线位置为：（　　）

A）`p.y + 1`　　B）`p.y - 1`　　C）`p.y`　　D）`p.x + 1`
