...
**2.** 灰度等级为256级，分辨率为2048*1024的显示器，至少需要的帧缓存容量为多少？
...
**3.** 用射线法判断一个点是否在多边形内时，若该射线与多边形的交点数目为（ ），则该点在多边形内部。
...
**5.** 用中点画线算法绘制直线段 $P_0(1,1)$ 到 $P_1(5,2)$，误差判别项 $d$ 的初值是：
...
**7.** 直线裁剪的 Liang-Barsky 算法中，“入点”的参数 $u_{max}=\max(0, u_k|p_k<0)$；“出点”的参数 $u_{min}=\min(1, u_k|p_k>0)$。下面错误的说法是：
...
**9.** 若对象空间有 k 个物体，图像空间的屏幕分辨率为 m*n，则图像空间消隐算法（如 Z-Buffer）的复杂度是：
...
**2. 改进的活动边表（AET）填充算法**
已知某多边形的顶点坐标为：$A(2, 1)$、$B(6, 1)$、$C(6, 5)$、$D(4, 3)$、$E(2, 5)$、$F(1, 4)$。若采用多边形扫描转换算法进行填充，且表结点结构为 $[y_{max} | x | 1/k | next]$。
请回答：
(1) 构建该多边形的完整边表（ET表）。
(2) 写出扫描线递增到 $y = 3$ 时的活动边表（AET表）状态。
(3) 写出 $y = 3$ 时的有效填充区间。

**3. Liang-Barsky 算法直线裁剪（10分）**
已知裁剪窗口为一个矩形区域，边界为：$x_{wmin} = 0$，$x_{wmax} = 2$，$y_{wmin} = 0$，$y_{wmax} = 2$。待裁剪直线段的起点坐标为 $A(1, -1)$，终点坐标为 $B(2, 3)$。
