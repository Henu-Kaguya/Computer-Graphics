...
**5. 对象空间有 $k$ 个物体，图像空间的屏幕分辨率为 $m \times n$，则图像空间消隐算法的复杂度是：** `[   ]`
...
**7. （______）明暗处理采用了法矢量双线性插值的方法先求出多边形内部各点法矢量再求颜色，（______）明暗处理采用了亮度双线性插值的方法求出多边形内部各点颜色。** `[   ]`
...
**10. Phong 光照明模型之漫反射光强 $I_d = I_p K_d (L \cdot N)$ 中 $K_d$ 是：** `[   ]`
...
**11. Phong 光照明模型之镜面反射光强 $I_s = I_p K_s \cos^n(\alpha)$ 中，$\alpha$ 是：** `[   ]`
...
**12. Phong 多边形着色方法中，点 $a$ 处的法向量插值公式 $N_a$ 是：** `[   ]`
...
**16. Z-Buffer 算法不仅需要帧缓冲区存放像素的亮度值，还需要一个 Z 缓冲区存放每个像素的深度值。** `(   )`
