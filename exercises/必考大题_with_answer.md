...
> **【参考答案】**
> - **基本思想**：当 $x$ 每次增加 $1$ 像素时，实际直线坐标 $y$ 相应的增量为斜率 $k$。当前像素纵坐标为 $y_i$，那么下一个备选像素坐标为 $y_i$（右侧像素）或 $y_i+1$（右上像素）。
> - 通过定义和跟踪理想直线纵坐标与当前绘制像素纵坐标之间的累积偏差（误差项），并与临界阈值 $0.5$ 像素宽度进行比较。若累积偏离值大于 $0.5$，纵坐标递增 $1$ 并对误差进行修正；否则纵坐标保持不变，依此迭代实现离散逼近。

*(2)* 设当前列决策变量为 $d_i$，推导下一列决策变量 $d_{i+1}$ 的误差判别递推公式。(4分)

> **【参考答案】**
> - **偏差表达式**：设理想直线为 $y = k(x_i+1) + c$。理想坐标与备选两像素（右、右上）的垂直偏差分别为：
>   $$
>   d_1 = y - y_i = k(x_i+1) + c - y_i
>   $$
>   $$
>   d_2 = (y_i+1) - y = y_i + 1 - k(x_i+1) - c
>   $$
> - **偏差差值**：
>   $$
>   d_1 - d_2 = 2k(x_i+1) - 2y_i + 2c - 1
>   $$
> - **消去分母引入决策变量**：令 $d_i = \Delta x(d_1 - d_2)$，代入 $k = \Delta y / \Delta x$ 得：
>   $$
>   d_i = 2\Delta y \cdot x_i - 2\Delta x \cdot y_i + C \quad (\text{其中 } C = 2\Delta y + 2\Delta x \cdot c - \Delta x)
>   $$
> - **递推公式推导**：下一列的决策变量为：
>   $$
...
> **【参考答案】**
> - **整数化改造**：通过将决策变量同乘以分母 $\Delta x$，消除了浮点数斜率 $k$。在递推过程中只有整数加减法（$\pm 2\Delta y$, $\pm 2\Delta x$）与乘二位移运算，从而彻底消除了浮点运算。
> - **整数型初值**：将起点 $(x_0, y_0)$ 代入初值式中（因为其在直线上满足 $y_0 = k x_0 + c$），可求得：
>   $$
>   d_0 = 2\Delta y - \Delta x
>   $$
> - **更新与选择决策依据**：
>   - 若 $d_i \ge 0$，选择右上相邻像素 $(x_i+1, y_i+1)$，且决策变量递增更新为：
>     $$
>     d_{i+1} = d_i + 2\Delta y - 2\Delta x
>     $$
>   - 若 $d_i < 0$，选择右下相邻像素 $(x_i+1, y_i)$，且决策变量递增更新为：
>     $$
